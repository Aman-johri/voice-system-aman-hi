...
app.post('/api/validateContactUPI', (req, res) => {
  ...
  if (!name || !mobile || !upi) {
    return res.status(400).json({ error: 'Name, mobile, and UPI ID are required' });
  }
  
  // Validate UPI format
  const upiPattern = /^[\w\-.]+@[\w]+$/;
  if (!upiPattern.test(upi)) {
    return res.status(400).json({ error: 'Invalid UPI ID format. Use format: name@upi' });
  }
  ...
  const upiName = upi.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '');
  ...
  setTimeout(() => {
    ...
    const validationRule1 = upiName.includes(contactName) || contactName.includes(upiName);
    ...
    if (!matches) {
      return res.status(400).json({ 
        error: 'UPI ID does not match the provided name and phone number. Please verify that the UPI ID belongs to this contact.',
        details: {
...
app.post('/api/validateUPI', (req, res) => {
  ...
  if (!upi || !mobile || !name) {
    return res.status(400).json({ error: 'UPI ID, mobile number, and name are required' });
  }
...
app.post('/api/addContact', (req, res) => {
  const { name, mobile, upi } = req.body || {};
  if (!name || !mobile || !upi) {
    return res.status(400).json({ error: 'Name, mobile, and UPI are required' });
  }
  const db = ensureDB();
  const exists = db.contacts.find(c => c.name.toLowerCase() === name.toLowerCase());
  if (exists) {
...
app.delete('/api/deleteContact/:name', (req, res) => {
  ...
  const db = ensureDB();
  const idx = db.contacts.findIndex(c => c.name.toLowerCase() === name.toLowerCase());
  if (idx === -1) {
